...
import { getSharedRedisClient } from "./shared-redis.js";
...
export class RedisCache {
	...
	async get<T>(key: string): Promise<T | undefined> {
		try {
			...
			logger.error(`Get error for ${this.prefix} cache, key "${key}"`, { error });
			return undefined;
		}
	}
	...
	async set(key: string, value: unknown, ttlSeconds?: number): Promise<void> {
		try {
			...
			logger.error(`Set error for ${this.prefix} cache, key "${key}"`, { error });
		}
	}
	...
	async delete(key: string): Promise<void> {
		try {
			...
			logger.error(`Delete error for ${this.prefix} cache, key "${key}"`, { error });
		}
	}
